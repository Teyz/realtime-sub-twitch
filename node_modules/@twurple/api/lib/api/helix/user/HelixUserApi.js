...
let HelixUserApi = HelixUserApi_1 = class HelixUserApi extends BaseApi_1.BaseApi {
    ...
    async getUserById(userId) {
        const users = await this._getUsers('id', [common_1.extractUserId(userId)]);
    ...
    async getUserByName(userName) {
        const users = await this._getUsers('login', [common_1.extractUserName(userName)]);
        return users.length ? users[0] : null;
    }
    /**
    ...
    async getFollows(filter) {
        ...
        return HelixPaginatedResult_1.createPaginatedResultWithTotal(result, HelixFollow_1.HelixFollow, this._client);
    }
    /**
     ...
     */
    async createFollow(fromUser, toUser, allowNotifications) {
        await this._client.callApi({
            ...
            jsonBody: {
                from_id: common_1.extractUserId(fromUser),
                to_id: common_1.extractUserId(toUser),
     ...
     */
    async deleteFollow(fromUser, toUser) {
        await this._client.callApi({
            type: 'helix',
            url: 'users/follows',
            method: 'DELETE',
    ...
    async getBlocks(user, pagination) {
        const result = await this._client.callApi({
            ...
            query: {
                broadcaster_id: common_1.extractUserId(user),
                ...HelixPagination_1.makePaginationQuery(pagination)
            }
        });
        return HelixPaginatedResult_1.createPaginatedResult(result, HelixUserBlock_1.HelixUserBlock, this._client);
    }
    /**
    ...
    async createBlock(target, additionalInfo = {}) {
        await this._client.callApi({
            ...
            query: {
                target_user_id: common_1.extractUserId(target),
    ...
    async getActiveExtensions(user) {
        const userId = user ? common_1.extractUserId(user) : undefined;
        const result = await this._client.callApi({
            type: 'helix',
    ...
    static _makeFollowsQuery(filter) {
        ...
        if (filter.user) {
            query.from_id = common_1.extractUserId(filter.user);
            hasUserIdParam = true;
        }
        if (filter.followedUser) {
            query.to_id = common_1.extractUserId(filter.followedUser);
            hasUserIdParam = true;
        }
        if (!hasUserIdParam) {
...
HelixUserApi = HelixUserApi_1 = tslib_1.__decorate([
    common_1.rtfm('api', 'HelixUserApi')
], HelixUserApi);
