...
let EventSubChannelRewardRemoveSubscription = class EventSubChannelRewardRemoveSubscription extends EventSubSubscription {
    ...
    async _subscribe() {
        if (this._rewardId) {
            return await this._client._apiClient.helix.eventSub.subscribeToChannelRewardRemoveEventsForReward(this._userId, this._rewardId, await this._getTransportOptions());
        }
        else {
            return await this._client._apiClient.helix.eventSub.subscribeToChannelRewardRemoveEvents(this._userId, await this._getTransportOptions());
        }
    }
};
