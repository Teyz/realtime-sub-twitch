...
let EventSubChannelRewardUpdateSubscription = class EventSubChannelRewardUpdateSubscription extends EventSubSubscription_1.EventSubSubscription {
    ...
    async _subscribe() {
        if (this._rewardId) {
            return await this._client._apiClient.helix.eventSub.subscribeToChannelRewardUpdateEventsForReward(this._userId, this._rewardId, await this._getTransportOptions());
        }
        else {
            return await this._client._apiClient.helix.eventSub.subscribeToChannelRewardUpdateEvents(this._userId, await this._getTransportOptions());
        }
    }
};
