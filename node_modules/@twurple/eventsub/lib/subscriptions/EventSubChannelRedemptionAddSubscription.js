...
let EventSubChannelRedemptionAddSubscription = class EventSubChannelRedemptionAddSubscription extends EventSubSubscription_1.EventSubSubscription {
    ...
    async _subscribe() {
        if (this._rewardId) {
            return await this._client._apiClient.helix.eventSub.subscribeToChannelRedemptionAddEventsForReward(this._userId, this._rewardId, await this._getTransportOptions());
        }
        else {
            return await this._client._apiClient.helix.eventSub.subscribeToChannelRedemptionAddEvents(this._userId, await this._getTransportOptions());
        }
    }
};
